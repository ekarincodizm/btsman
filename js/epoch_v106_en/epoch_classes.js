...
function Epoch(name,mode,targetelement,multiselect, yAdjusted)
{
	this.state = 0;
	this.name = name;
	...
	//------------- Netta Modified ----------------------
	if (!yAdjusted) { // the default date format to use - can be customized to the current locale
	    yAdjusted = 0;
	}
	this.yearAdjusted = yAdjusted;
...
//------------- Netta Modified ----------------------
Epoch.prototype.updatePosition = function() {
this.topOffset = this.tgt.offsetHeight; // the vertical distance (in pixels) to display the calendar from the Top of its input element
this.leftOffset = 0; 					// the horizontal distance (in pixels) to display the calendar from the Left of its input element
...
//this.calendar.style.top = newoffset + 'px';
}
//-----------------------------------------------------------------------------
Epoch.prototype.calConfig = function() //PRIVATE: initialize calendar variables
{
    ...
    var dnow = new Date();
    this.rangeYearLower = dnow.getFullYear() - 100;
    this.rangeYearUpper = dnow.getFullYear() + 20;
...
Epoch.prototype.createMainHeading = function() //PRIVATE: Creates the primary calendar heading, with months & years
{
    ...
    for (i = this.rangeYearLower; i <= this.rangeYearUpper; i++) {
        ...
        //------------- Netta Modified ----------------------
        opt.appendChild(document.createTextNode(i + this.yearAdjusted));
...
Epoch.prototype.outputDate = function(vDate, vFormat) //PUBLIC: outputs a date in the appropriate format (DEPRECATED)
{
    ...
    var vYearShort = this.addZero(vYearLong.toString().substring(3, 4));
...
CalCell.prototype.onclick = function() {
    ...
    if (!owner.selCurMonthOnly || cell.date.getMonth() == owner.displayMonth && cell.date.getFullYear() == owner.displayYear) {
        ...
        else //if we can only select one cell at a time
        {
            ...
            {
                owner.tgt.value = owner.selectedDates[0].dateFormat('d/m/Y', owner.yearAdjusted);
                if (owner.mode == 'popup') {
                    owner.hide();
                }
            }
...
//-----------------------------------------------------------------------------
Date.prototype.dateFormat = function(format, adjY) {
    ...
    var c = "";
    var token = "";
    var y = (this.getFullYear() + adjY).toString();
